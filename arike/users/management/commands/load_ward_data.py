...
from arike.users.models import LOCAL_BODY_CHOICES, District, LSG_Body, Ward
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options) -> Optional[str]:
        ...
        def get_local_body(lb):
            if not lb["district"]:
                return None
            return LSG_Body.objects.filter(
                name=lb["name"],
                district=district_map[lb["district"]],
                lsg_body_code=lb.get("localbody_code"),
                kind=LOCAL_BODY_CHOICE_MAP.get((lb.get("localbody_code", " "))[0], LOCAL_BODY_CHOICES[-1][0]),
            ).first()
            ...
            with open(f"{f}", "r") as data_f:
                ...
                if data.get("district") is not None:
                    ...
                    for ward in wards:
                        counter += 1
                        try:
                            obj = Ward(lsg_body=local_body, number=get_ward_number(ward), name=get_ward_name(ward))
                            obj.save()
