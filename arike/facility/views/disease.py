...
from arike.facility.models.patients import PatientDisease, Patient
from arike.facility.forms.patients import DiseaseForm
from arike.facility.views.family import AuthView

class CreateDisease(AuthView):
  ...
  def post(self, request, pk1):
    try:
      ...
      if form.is_valid():
        ...
        model_obj.save()
        return HttpResponseRedirect(reverse("arike:view-disease", args=(pk1,)))
      return render(request, self.template, {"form": form})
    except Exception as e:
        logging.error(e)
        return HttpResponseRedirect("/500")

class UpdateDisease(AuthView):
  model = PatientDisease
  template = "disease/update.html"
  form_class = DiseaseForm
...
class ViewDisease(AuthView):
  model = PatientDisease
  template = "disease/view.html"
...
class DeleteDisease(AuthView):
  model = PatientDisease
  template = "disease/delete.html"

  def get(self, request, pk, pk1):
    try:
      data = self.model.objects.get(id=pk, patient_id=pk1).disease
